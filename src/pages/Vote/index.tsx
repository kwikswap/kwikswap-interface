...
import { KWIK, ZERO_ADDRESS } from '../../constants'
import { JSBI, TokenAmount, ChainId } from '@kwikswap/sdk'
...
export default function Vote() {
  ...
  const kwikBalance: TokenAmount | undefined = useTokenBalance(
    account ?? undefined,
    chainId ? KWIK[chainId] : undefined
  )
  const userDelegatee: string | undefined = useUserDelegatee()

  // show delegation option if they have have a balance, but have not delegated
  const showUnlockVoting = Boolean(
    kwikBalance && JSBI.notEqual(kwikBalance.raw, JSBI.BigInt(0)) && userDelegatee === ZERO_ADDRESS
  )
      ...
      <TopSection gap="md">
        <VoteCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>Kwikswap Governance</TYPE.white>
              </RowBetween>
              <RowBetween>
                <TYPE.white fontSize={14}>
                  KWIK tokens represent voting shares in Kwikswap governance. You can vote on each proposal yourself or
                  delegate your votes to a third party.
                </TYPE.white>
              </RowBetween>
              <ExternalLink
                style={{ color: 'white', textDecoration: 'underline' }}
                href="https://kwikswap.org/blog/kwik"
                target="_blank"
              >
                <TYPE.white fontSize={14}>Read more about Kwikswap governance</TYPE.white>
              </ExternalLink>
            </AutoColumn>
          </CardSection>
          <CardBGImage />
