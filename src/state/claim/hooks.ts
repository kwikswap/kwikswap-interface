...
import { TokenAmount, JSBI, ChainId } from '@kwikswap/sdk'
...
function fetchClaim(account: string, chainId: ChainId): Promise<UserClaimData | null> {
  ...
  return (CLAIM_PROMISES[key] =
    CLAIM_PROMISES[key] ??
    fetch(`https://gentle-frost-9e74.kwikswap.workers.dev/${chainId}/${formatted}`)
      .then(res => {
        if (res.status === 200) {
          return res.json()
        } else {
...
// check if user is in blob and has not yet claimed KWIK
...
export function useUserUnclaimedAmount(account: string | null | undefined): TokenAmount | undefined {
  ...
  if (!canClaim || !userClaimData) {
    return new TokenAmount(kwik, JSBI.BigInt(0))
  }
  return new TokenAmount(kwik, JSBI.BigInt(userClaimData.amount))
}
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} KWIK`,
