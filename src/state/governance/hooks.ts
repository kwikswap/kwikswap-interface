...
import { TokenAmount } from '@kwikswap/sdk'
import { isAddress } from 'ethers/lib/utils'
import { useGovernanceContract, useKwikContract } from '../../hooks/useContract'
...
import { abi as GOV_ABI } from '@kwikswap/governance/build/GovernorAlpha.json'
...
export function useUserDelegatee(): string {
  ...
  const kwikContract = useKwikContract()
  const { result } = useSingleCallResult(kwikContract, 'delegates', [account ?? undefined])
...
export function useUserVotes(): TokenAmount | undefined {
  ...
  const votes = useSingleCallResult(kwikContract, 'getCurrentVotes', [account ?? undefined])?.result?.[0]
  return votes && kwik ? new TokenAmount(kwik, votes) : undefined
}
...
export function useUserVotesAsOfBlock(block: number | undefined): TokenAmount | undefined {
  ...
  // check for available votes
  const kwik = chainId ? KWIK[chainId] : undefined
  const votes = useSingleCallResult(kwikContract, 'getPriorVotes', [account ?? undefined, block ?? undefined])
    ?.result?.[0]
  return votes && kwik ? new TokenAmount(kwik, votes) : undefined
}
...
export function useDelegateCallback(): (delegatee: string | undefined) => undefined | Promise<string> {
  ...
  return useCallback(
    (delegatee: string | undefined) => {
      ...
      if (!kwikContract) throw new Error('No KWIK Contract!')
      return kwikContract.estimateGas.delegate(...args, {}).then(estimatedGasLimit => {
        return kwikContract
    ...
    [account, addTransaction, chainId, library, kwikContract]
  )
}
